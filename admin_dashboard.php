...
<body>
    ...
    <main>
        ...
        <div class="mb-4">
            <a href="regional_stats.php" class="btn btn-info text-white">View Regional Impact Analysis</a>
        </div>
            ...
            <tbody>
                ...
                if (mysqli_num_rows($pending_students) > 0) {
                    while ($row = mysqli_fetch_assoc($pending_students)) {
                        ...
                        echo "<td>" . htmlspecialchars($row['upazila']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['verification_status']) . "</td>";
                        echo "<td>
                <a href='verify_student.php?id=" . urlencode($row['student_id']) . "&action=verify' class='btn btn-sm btn-success'>Verify</a>
                <a href='verify_student.php?id=" . urlencode($row['student_id']) . "&action=reject' class='btn btn-sm btn-danger'>Reject</a>
            </td>";
                        echo "</tr>";
                    }
                } else {
                ...
                if (mysqli_num_rows($applications) > 0) {
                    while ($row = mysqli_fetch_assoc($applications)) {
                        ...
                        if ($row['status'] == 'approved') {
                            echo "<a href='disburse_funds.php?app_id={$row['application_id']}' class='btn btn-sm btn-outline-success'>Disburse</a>";
                        }
                        echo "</td>";
                        echo "</tr>";
                    }
